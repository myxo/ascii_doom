...
void move_player(int forward, int right, float time_elapsed) {
    ...
    if (!is_wall(new_x, world->player.pos.y, world->player.radius) && fabs(new_x - world->player.pos.x) > 0.015)
    ...
    if (!is_wall(world->player.pos.x, new_y, world->player.radius) && fabs(new_y - world->player.pos.y) > 0.015)
